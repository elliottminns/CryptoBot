...
module.exports = {
  load() {
    ...
    return {
      ...
      async placeBuyOrder({ product, amount, price, cost }) {
        const currency = this.currencyForProduct(product)
        const order = await bitstamp.buyMarketOrder(amount, currency)
        return order
      },
      async placeSellOrder({ product, amount, cost }) {
        const currency = this.currencyForProduct(product)
        const order = await bitstamp.sellMarketOrder(amount, currency)
        return order
      },
